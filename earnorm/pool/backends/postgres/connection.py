...
This module provides a connection implementation for PostgreSQL.
...
Example:
    ...
    ```python
    from earnorm.pool.backends.postgres import PostgresConnection
...
class PostgresConnection(ConnectionProtocol[DBType, CollType]):
    """PostgreSQL connection implementation.

    Examples:
        >>> conn = PostgresConnection(database="test")
        >>> await conn.ping()
        True
        >>> await conn.execute("SELECT * FROM users WHERE id = $1", [1])
        [{"id": 1, "name": "test"}]
    """
    ...
    def get_database(self) -> DBType:
        ...
        Returns:
            DBType: PostgreSQL database instance
    ...
    def get_collection(self, name: str) -> CollType:
        ...
        Returns:
            CollType: PostgreSQL collection instance
